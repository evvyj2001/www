$(document).ready(function(){
        ...
        $('.content_area .tab_content').css('overflow','hidden');
        $('.content_area .tab_content:eq(0)').show(); //첫번째 내용만 보여라
        $('.content_area .tab_content:eq(0)').css('height','auto');
        $('.content_area .tab_content').css('overflow','auto');
        $('.title_area .tab1').addClass('current');
        //첫번째 tab에 .on을 추가

        $('.title_area .tab').each(function (index) {  // index=> 0 1 2
            $(this).click(function(){   //각각의 탭메뉴를 클릭하면 
                $('.content_area .tab_content').css('height','auto');
                $('.content_area .tab_content').css('overflow','auto');
